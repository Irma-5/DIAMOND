...
def main():
    ...
    IMAGESDICT = {'uncovered goal': pygame.image.load('RedSelector.png'),
                  ...
                  'girl2': pygame.image.load('girl1.png'),
    ...
    PLAYERIMAGES = [IMAGESDICT['boy2'],
                    ...
                    IMAGESDICT['dino'],
                    IMAGESDICT['among']]
    startScreen() # меню игры
    levels = readLevelsFile('DiamondLevels.txt') # чтение карт
            ...
            if event.type == QUIT:
                ...
                if event.key == K_LEFT:
                    ...
                    if currentImage >= len(PLAYERIMAGES):
                        currentImage = 0
                    mapNeedsRedraw = True
            elif event.type == KEYUP: # при нажатии этих клавиш камера будет двигаться
                if event.key == K_a:
                    cameraLeft = False
                elif event.key == K_d:
                    cameraRight = False
...
def decorateMap(mapObj,
                startxy):  # в этой функции отрисовываются различные деревья и тд, дополняющие карту до фиксированного размера
...
def startScreen():  # Зарисовка начального экрана и правила
    ...
    instructionText = ['Доведите алмазики до цели, чтобы пройти уровень',
                       ...
                       'стрелочки ("K_LEFT" и тд) для управления персонажем',
                       'Пробел для обновления уровня, Esc для выхода.',
